...
def main(args, data):
    ...
    for frac in frac_array:
        ...
        seed_array = np.random.randint(100000, size=N_sample)
        
        for seed in seed_array:
            print(f"Processing set for mu = {frac} with seed = {seed}")
            # Create jet data. The 'createJetData' function is assumed to use the
            # provided data object to return the full set of jets.
            alljet_data, _ = createJetData("all", True, set_mu=frac, seed=seed,
                                           n_param=[1, 1, 1, 1, 1, 0],
                                           useRand=True, data=data)
            ...
            mu = compute_mu_nuan_2NP_class(data_2j,data_1j, class_model_load,
                                         bin_splines_S_class,bin_splines_BG_class)
